...
def parse_vcf(vep_vcf, sample_name, normal_name):
    ...
    for record in vcf_reader:
        ...
        af = (
            record.genotype(sample_name)["AF"]
            if "AF" in str(record.genotype(sample_name))
            else "NA"
        )
        dp = (
            record.genotype(sample_name)["DP"]
            if "DP" in str(record.genotype(sample_name))
            else "NA"
        )

        raf = (
            record.genotype(sample_name)["RAF"]
            if "RAF" in str(record.genotype(sample_name))
            else "NA"
        )
        rdp = (
            record.genotype(sample_name)["RDP"]
            if "RDP" in str(record.genotype(sample_name))
            else "NA"
        )

        gx = (
            record.genotype(sample_name)["GX"]
            if "GX" in str(record.genotype(sample_name))
            else "None"
        )
        if str(gx).find("|") != -1:
            gx = str(gx).split("|")[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sample_name", required=True, type=str, help="Tumor sample name"
    )
    parser.add_argument(
        "--normal_name", required=False, default="", type=str, help="Normal sample name"
    )
    parser.add_argument(
        "--vep_vcf", required=True, type=_file_read, help="VEP annotated VCF"
    )
    parser.add_argument(
        "--pep_fasta",
        required=True,
        type=_file_read,
        help="FASTA file with chopped peptides",
    )
    parser.add_argument(
        "--mixMHC2pred_result",
        required=True,
        type=_file_read,
        help="mixMHC2pred result file",
    )
    parser.add_argument(
        "--out", required=True, type=_file_write, help="Parsed mixMHC2pred result"
    )
    ...
    for line in mixMHC2pred_result:
        ...
        if line.startswith("Peptide"):
            ...
            continue
        key = (
            peptide_records[i]["gene_symbol"]
            + "_"
            + peptide_records[i]["enst"]
            + "_"
            + peptide_records[i]["variant"]
        )
